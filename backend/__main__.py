...
import argparse
import structlog
from backend.symbol_finder import SymbolExtractor
from backend.LLMs import Claude, ChatGPT, Ollama
...
structlog.configure(
    ...
    logger_factory=structlog.WriteLoggerFactory(
        file=Path('code_safe').with_suffix(".log").open("wt")
    )
)
...
log = structlog.get_logger("code_safe")
...
def initialize_llm(llm_arg: str, system_prompt: str = "") -> Claude | ChatGPT | Ollama:
    ...
    if llm_arg == 'claude':
        anth_model = os.getenv("ANTHROPIC_MODEL", "claude-3-sonnet-20240229")
